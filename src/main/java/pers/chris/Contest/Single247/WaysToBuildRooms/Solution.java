package pers.chris.Contest.Single247.WaysToBuildRooms;

import java.util.*;

class Solution {
    Map<Integer, List<Integer>> edges = new HashMap<>();
    Queue<Integer> optionalRooms = new LinkedList<>();
    ...
    public int waysToBuildRooms(int[] prevRoom) {
        ...
        for (int i = 1; i < len; i ++) {
            List<Integer> rooms = edges.getOrDefault(prevRoom[i], new ArrayList<>());
            rooms.add(i);
            edges.put(prevRoom[i], rooms);
        }

        optionalRooms.addAll(edges.get(0));
        seq.add(0);
        backTrack(optionalRooms);
        return sum;
    }

    public void backTrack (Queue<Integer> optionalRooms) {
        if (len == seq.size()) {
//            System.out.println(sum);
        ...
        for (int i = 0; i < optionalRooms.size(); i ++) {
            int room = optionalRooms.poll();
            seq.add(room);
            optionalRooms.addAll(edges.getOrDefault(room, new ArrayList<>()));
            backTrack(optionalRooms);

            seq.remove(room);
            optionalRooms.removeAll(edges.getOrDefault(room, new ArrayList<>()));
    ...
    public static void main(String[] args) {
        ...
        System.out.println(solution.waysToBuildRooms(prevRoom));
    }
}
